...
//
// 10.12 19:30 美团 ES数据一致性 RocketMQ 延迟队列 CPU负载 SQL key_length 大促时遇到的问题 Future并非异步 CompletableFuture如何做到异步
//
// 10.15 11:00 新浪 CAP与数据库（关系型，非关系型）的关系，与zookeeper的关系 百万级秒杀如何实现 监控，无侵入
//                 数据库，行式存储和列式存储 HTTPS过程 跨站的单点登录    java进程内存管理，堆之外  jar包冲突发生在什么时候，编译过程中发生了什么
// 10.26 19:00 百度 动态代理具体实现 k8s docker 基本概念
// 10.29 16:00 腾讯 dubbo 动态无感平滑上线 linux统计命令(awk) 方法级别的运行问题如何定位  MySQL执行效果分析(表的统计信息) binlog
// 11.10 21:00 深拷贝浅拷贝
//
//
...
//        rCountDownLatch.trySetCount(3);
    ...
    public String[][] topKstrings (String[] strings, int k) {
        ...
        List<javafx.util.Pair<String, Integer>> sortList = numMap.entrySet().stream().map(entry -> new javafx.util.Pair<String, Integer>(entry.getKey(), entry.getValue()))
